...
static void sm3_compress(uint32_t digest[sm3_block_BYTES / sizeof(uint32_t)], const uint8_t block[sm3_block_BYTES]) {
	...
	std::vector<std::thread> threads; //存储线程函数的变量,利用多线程加速循环
	//for (j = 0; j < 16; j++) W[j] = byte_swap32(pblock[j]);
	for (int j = 0; j < 8; j++)
	{
		threads.push_back(std::thread(func_swap32, j, pblock[j], j + 8, pblock[j + 8], W));
	}
	...
	for (int j = 0; j < 8; j++)
	{
		threads.push_back(std::thread(func_1, j + 16, j + 24, j + 32, j + 40, j + 48, j + 56, W));
	}
	threads.push_back(std::thread(func_1, 64, 65, 66, 67, 0, 0, W));
	for (auto& thread : threads) {
		thread.join();	//执行线程变量中的每个线程执行函数
	}
	...
	for (int j = 0; j < 8; j++)
	{
		threads.push_back(std::thread(func_2, j, j + 8, j + 16, j + 24,j+32,j+40,j+48,j+56,W1, W));
	}
	for (auto& thread : threads) {
		thread.join();	//执行线程变量中的每个线程执行函数
	}
