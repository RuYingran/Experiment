...
def sm3_LEA(mes1,mes2):
    #对mes1+padding+mes2进行hash
    mes1_pad=padding(mes1)
    left=(len(hex(mes2))-2)*4
    res1=(int(mes1_pad,2)<<left)+mes2
    hash_value1=sm3(res1)
    print("hash_value1=",hash_value1)
    
    #先对mes1进行hash，将得到的值作为IV对mes2+padding+length进行hash
    cipher=sm3(mes1)
    iv_=int(cipher,16)
    mes2_pad=padding(mes2)
    l=len(mes2_pad)
    res2_=''
    res2=int(mes2_pad,2)+l
    for i in hex(res2)[2:]:
        res2_=res2_+'{:04b}'.format(int(i,16))
    hash_value2=hex(int(iterfunction_LEA(res2_,iv_),2))
    print("hash_value2=",hash_value2)

    #得到的hash值相同则代表可实现长度扩展攻击
    if(hash_value1==hash_value2):
        print("Length Extension_attack Succeed")
    else:
        print("Length Extension_attack Failed")

if __name__ == '__main__':
    start=time.time()
    sm3_LEA(0x0125,0x1029)
    end=time.time()
